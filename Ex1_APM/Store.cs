...
namespace Ex1_APM
{
	...
	public class Store
	{
		...
		/// </summary>
		private static volatile Store _instance = new Store();
		...
		public bool Register(string fileName, IPEndPoint client)
		{
			lock (_instance)
			{
				HashSet<IPEndPoint> fileHosts;
				if (!_store.ContainsKey(fileName))
				{
					fileHosts = new HashSet<IPEndPoint>();
					_store[fileName] = fileHosts;
				}
				else
		...
		public bool Unregister(string fileName, IPEndPoint client)
		{
			...
			lock (_instance)
			{
				...
				var locations = _store[fileName];
				var result = locations.Remove(client);
